...
# Volcano plot.
# ---------------------
deseq_res <- read.csv(here("data/deseq2_result.csv"), header = TRUE, row.names = 1)
...
genes_to_label <- c("NFE2L2","NOX2","NOX4","DUOX1","DUOX2","KEAP1","CAT",
                    "GPX1", "GSS","SOD1","SOD2","TRX", "CYGB",
                    "MAT1A", "GLS2","LPK","MAT2A","GLS1","PKM2", 
                    "ESRP2", "SRSF3", "SLU7","HNF4A","HNF1A","CEBPA","FOXA2",
                    "FOXO1", "PCK1", "G6PC1")

# plot
save_plot(plot = volcano_plot(df = replaced_res), 
          filename = "volcanoPlot.png", height = 7, width = 5.5)
save_plot(plot = volcano_plot(df = replaced_res, label_gene = genes_to_label), 
...
# nrf2 df
nrf2_heatmap_df <- getHeatmapData(tidy_data = tidy_data, deseq_res = deseq_res, gmt_file_path = here("reference/WP_NRF2_PATHWAY.v2024.1.Hs.gmt"))

# z-score normalization (mean = 0, dispersion = 1), i.e. standardization
nrf2_heatmap_z <- genefilter::genescale(nrf2_heatmap_df, axis = 1, method = "Z") 

# plot 
nrf2_heatmap <- plot_heatmap(data = as.matrix(nrf2_heatmap_z[, desired_order]), color = my_color, annotation_col = annotation_col, annotation_colors = ann_colors)

# save plot
save_plot(plot = nrf2_heatmap, filename = "nrf2_heatmap.png", width = 6, height = 9)

# ---------------------
# FXR heatmap
# ---------------------
FXR_heatmap_df <- getHeatmapData(tidy_data = tidy_data, deseq_res = deseq_res, gmt_file_path = here("reference/BIOCARTA_FXR_PATHWAY.v2024.1.Hs.gmt"))

# z-score normalization (mean = 0, dispersion = 1), i.e. standardization
FXR_heatmap_z <- genefilter::genescale(FXR_heatmap_df, axis = 1, method = "Z") 
...
# PPARG heatmap
# ---------------------
pparg_heatmap_df <- getHeatmapData(tidy_data = tidy_data, deseq_res = deseq_res, gmt_file_path = here("reference/PPARG_01.v2024.1.Hs.gmt"))

# z-score normalization (mean = 0, dispersion = 1), i.e. standardization
pparg_heatmap_z <- genefilter::genescale(pparg_heatmap_df, axis = 1, method = "Z") 

# plot
pparg_heatmap <- plot_heatmap(data = as.matrix(pparg_heatmap_z[, desired_order]), color = my_color, annotation_col = annotation_col, annotation_colors = ann_colors)
 
# save plot
save_plot(plot = pparg_heatmap, filename = "pparg_heatmap.png", width = 6, height = 9)
...
# genelists
GENE_LISTS <- list(NRF2 = c("ENSG00000116044","ENSG00000079999"), 
                   NOX = c("ENSG00000165168","ENSG00000086991","ENSG00000137857", "ENSG00000140279"),
                   ROS = c("ENSG00000121691","ENSG00000233276","ENSG00000100983","ENSG00000142168", "ENSG00000291237","ENSG00000136810"),
...
names(GENE_LISTS$NRF2) <- c("NFE2L2", "KEAP1")
names(GENE_LISTS$NOX) <- c("NOX2", "NOX4", "DUOX1", "DUOX2")
names(GENE_LISTS$ROS) <-  c("CAT","GPX1", "GSS","SOD1", "SOD2", "TRX")
names(GENE_LISTS$CYGB) <- "CYGB"
names(GENE_LISTS$ADULT_ISOZYME) <- c("MAT1A", "GLS2", "LPK")
names(GENE_LISTS$FETAL_ISOZYME) <- c("MAT2A", "GLS1", "PKM2")
